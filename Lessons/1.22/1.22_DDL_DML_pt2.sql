...
CREATE TEMPORARY TABLE portugal_jobs_flat_temp AS
SELECT *
FROM 
    staging.priority_jobs_flat_view
WHERE job_location = 'Portugal';

SELECT 
    job_title_short,
    COUNT(*) AS job_count
FROM portugal_jobs_flat_temp
GROUP BY job_title_short
ORDER BY job_count DESC;

SELECT COUNT(*) FROM staging.job_postings_flat;
SELECT COUNT(*) FROM staging.priority_jobs_flat_view;
SELECT COUNT(*) FROM portugal_jobs_flat_temp;
...
--TRUNCATE
-- É necessário voltar a correr a primeira query!
